/* Apaga tabela de Contatos se existir */
...
/* Apaga trigger ValidarContatos */  
DROP TRIGGER IF EXISTS ValidarContatos;

/* Cria trigger ValidarContatos */  
...
BEGIN 
  SELECT 
    CASE
      ...
      WHEN 0 < (SELECT COUNT(*) FROM Contatos WHERE Nome = NEW.Nome) THEN RAISE (ABORT, "Contato já cadastrado! " )
      WHEN (NEW.TipoMovimento IS NULL) OR (New.TipoMovimento = '') THEN RAISE (ABORT,"Informe um tipo de movimento! ") 
    
    END;
END;

/* Insere dados na tabela Contatos*/  
INSERT INTO Contatos (Nome,TipoMovimento) VALUES ("TRABALHO","R"), ("NETFLIX","D");
...
/* Apaga trigger ValidarCategorias */					 
DROP TRIGGER IF EXISTS ValidarCategorias;

/* Cria trigger ValidarCategorias */					 
...
/* Cria trigger GerarPrimeiraParcela */
...
/*Exibe vencimento e valor com formatação */
SELECT 
   strftime('%d/%m/%Y',DataVencimento) Vencimento, -- Formatar data 
   Replace(printf("R$ %.2f", Valor),'.',',') AS Valor  -- Formatar valor
 FROM contas;
